...
describe('public Signal feeds', () => {
    ...
    test('contain every checked live release and only owned clean links', () => {
        ...
        for (const item of feed.items) {
            ...
            expect(item.content_text).toMatch(/(?:Media note:|Visual note: Mythical Void procedural star-map artwork|Visual note: Text-only release note about the live browser game|Visual note: the earlier media was withheld after human review)/);
        }
    });
