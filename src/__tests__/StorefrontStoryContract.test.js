...
const choiceReview = require('../../docs/company/content/PROJECT_BEACON_CHOICE_PROOF_REVIEW.json');
...
describe('storefront Project Beacon story contract', () => {
    ...
    test('provides a truthful public press and creator room', () => {
        ...
        expect(gameplayManifest.captures).toHaveLength(17);
        ...
        expect(gameplayManifest.captures.filter(capture => (
            ['GP-016', 'GP-017'].includes(capture.id)
        ))).toHaveLength(2);
        expect(choiceReview.publicUseApproved).toBe(false);
        expect(choiceReview.reviewState).toBe(
            'authentic_spoiler_safe_supporting_proof_ready_for_kevin_review'
        );
