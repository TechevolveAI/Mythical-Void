...
4. In progress: the separate antagonist outcome, atomic local reward receipt and compact result/ending adapter are implemented privately. Complete the approach route and authored phase presentation, connect that adapter to the real fight and campaign, then bind the prepared films to actual story cues.
...
3. Campaign adapter: the separate antagonist completion policy is now implemented (see below), but not registered in production or connected to the private fight scene yet. Keep `PlatformerLevelScene.completeLevelProgression` unchanged for the other levels. Preserve Nova, `command_module`, existing rewards, and canonical level IDs.
4. Connect the durable receipt to the actual final hit and present only after grounded recovery. Real fight-to-Sanctuary/repair/choice browser validation remains pending. The isolated result proof is not that whole journey.
