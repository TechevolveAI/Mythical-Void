...
4. Implemented privately: the approach, separate antagonist outcome, atomic local reward receipt and compact result/ending adapter connect the real fight to the existing campaign ending. Arrival/victory film cues have fixture-media proof. Authored phase presentation and actual reviewed films remain outstanding.
...
## Queued reward recovery repair, 2026-09-21

The queued-inventory refresh window described in the earlier milestones is now repaired on this private branch. The new regression first reproduced three saves after freeing a slot: empty slot plus pending reward, owned reward plus the same pending reward, then owned reward with the queue cleared. Refreshing the middle snapshot could duplicate a boss reward.

`InventoryManager.claimPendingBossRewards` now stages item additions and the remaining inbox without changing live stacks. It publishes both in one complete `inventory` update and calls the existing save boundary once for the transfer. The preceding slot-removal save is still separate and safe: it contains only the pending reward. Item notifications occur after the complete transfer, and recursive collection from a listener is guarded. Normal item purchase/use behavior, stack rules, capacity, item definitions, other inventory metadata and save schema remain unchanged.

This is a shared inventory correctness repair, not a finale-only queue format. Regression coverage includes all six authored boss rewards, partial collection, stacking at capacity, untouched source arrays, state/item listener re-entry and denied persistence. Tests use the actual GameState serializer, local-storage writes, validator and restore path to reopen every saved stage of the Trumptopus transfer. Each restored game contains exactly one Super Blast, unchanged coins, one Command Module, the same receipt and creature identity. An injected failure on the transfer's primary storage write keeps a coherent in-session result; the last durable snapshot retains the queue and can claim once when storage returns. It does not promise durability when browser storage is unavailable.

Focused checks pass 6 suites / 73 tests; the full suite passes 257 suites / 2,514 tests. Evidence is separate in `.visual-review/trumptopus-reward-recovery/` and binds the exact local commit, test reports, source hashes and production build. This slice changes no screen, geometry, controls or audio, so it does not rerun or re-label earlier visual proof as new evidence. No browser, server, provider or live-service test is needed for these deterministic save checks.

The existing `GameState.save()` path and its existing opt-in cloud behavior are not removed or extended. No direct cloud-sync hook has been added to the private victory adapter; that migration/consent decision remains open. The inventory repair compiles into a normal build but is not deployed. Production, public media and release authority remain unchanged. Missing artist-prepared Trumptopus layers, actual reviewed finale films and human play/art approval still prevent release.

## Save-compatible rollback rehearsal, 2026-09-21

`npm run verify:trumptopus-rollback` now executes the pinned pre-finale GameState reader/writer against candidate saves and then restores them with the current reader. Eleven cases cover old campaigns, incomplete approach/fight progress, a win, queued reward collection, real historical Empress outcomes, all three canonical ending states and an unknown future run schema. The test uses a read-only existing identity fixture and isolated in-memory storage, never player saves or live services.

The negative control proves that the bare `fd4f3f18` artifact is not a suitable rollback after new-finale play: it accepts the save but falsely infers an Empress rescue. The supported fallback retains the legacy encounter plus the outcome and inventory compatibility repairs. Explicit Empress history survives, new Trumptopus wins do not become Empress rescues, and round-tripping the old save writer preserves the new additive state. The existing scene registration, Final Void level, repair and ending source remain unchanged.

See [rollback contract](TRUMPTOPUS_ROLLBACK.md) for the exact retained files and future release procedure. Evidence is private in `.visual-review/trumptopus-rollback/`. This does not claim a hosted rollback, physical-device or cloud round trip, three browser ending journeys, or a tested future production feature switch. It is a prerequisite for that later integration. No game source, public content, generation request, push or deployment changes in this slice.

## Connected tentacle motion, 2026-09-21

Kevin clarified that extending tentacles should be part of the attack. Inspection of the private attack pose found an actual discontinuity: at 390px with a locked target of x=100, ready-to-warning jumped 160.23px for a grasp and 164.34px for a sweep. The temporary limb also used a rigid two-segment elbow.

The private finale now derives a connected, tapered motion guide from the same contact pose used by collision. Warning reaches smoothly from rest, a bend travels toward the tip during extension, and recoil returns to that same rest position. Grasp/sweep state boundaries no longer teleport the tip or its connected guide. The closing grasp retains two limbs. This is motion preparation for the later authored layers, not a recreation of the supplied image or finished character artwork.

Existing attack timings, target lock, damaging contact footprints, exposed windows and phase health are unchanged. The sweep stays low through contact and exposure before returning upward. Only its safe exposed-pose placement changes slightly; the damaging sweep remains the same. The curved connecting tissue itself does not introduce a new invisible damage area. Other levels, approach geometry, saves, films and production registration are unchanged.

Five focused suites / 59 checks cover the encounter, phase adapter, campaign adapter, connected geometry, state continuity at phone/desktop widths, bounded outline, travelling deformation and unchanged contact rules. The existing silent real-input fight proof can write to a separate named private folder so earlier exact-source captures remain intact: `TRUMPTOPUS_PROOF_NAME=trumptopus-tentacle-motion npm run smoke:trumptopus-finale`. Its `report.json` is authoritative about browser completion; this entry alone does not assert a passed capture. No paid asset generation or character-art approval is implied.

The first browser run at `fdb75d45` completed the phone fight but failed a desktop sweep dodge. A follow-up at `e1f1c176` recorded a phone jump arriving 300ms into the 750ms strike, after collision at 217ms: observing launch and then delivering native input was too late in that run. Both failures are retained separately, not passed off as a successful pair. The proof now reacts during the latter half of the visible wind-up, allowing native input delivery time. The unchanged -420 jump velocity and 500 gravity provide sufficient airtime for the remaining sweep. Actual jump state/progress is recorded. The zero-damage dodge assertion, real-input requirement and all fight/reward conditions remain intact. No attack speed, collision or player physics was changed to pass the proof.

The next run completed the phone fight and avoided desktop sweep damage, but missed its desktop counter. Capturing a still before issuing attacks consumes the same exposed window being tested. Per-phase stills now happen only after a successful counter, with filenames stating `countered` rather than claiming an exposed pose. Continuous recordings retain the full action, and native desktop attack timing is logged alongside jump timing. This changes proof observation order, not the fight or required successful hit. The failed evidence remains separate.
