...
The first browser run at `fdb75d45` completed the phone fight but failed a desktop sweep dodge. That evidence is retained separately, not passed off as a successful pair. The proof had waited until late in the 750ms sweep before sending jump input. With the unchanged -420 jump velocity and 500 gravity, a launch-time jump remains above this low sweep throughout its crossing; the proof now uses that readable cue and records actual jump-input state/progress. The zero-damage dodge assertion, real-input requirement and all fight/reward conditions remain intact. No attack speed, collision or player physics was changed to pass the proof.
