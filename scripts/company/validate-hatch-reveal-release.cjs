...
requireValue(
    scene.includes('const targetScale = layout.creatureScale')
        && scene.includes('creatureScale: isCompact ? Math.min(3.05, width / 125) : 3.15')
        && scene.includes('creature.clearMask?.(true)')
        && !scene.includes('creature.setMask('),
    'large intact creature presentation rule is missing'
);
