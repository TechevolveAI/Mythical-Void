...
requireValue(evidence.state === 'crawl_foundation_live_public_discovery_not_observed', 'Live search evidence must retain the observed crawlable-but-not-discovered state.');
...
requireValue(crawl.sitemapStatus === 200 && crawl.sitemapRouteCount === 8, 'Live sitemap evidence must retain eight reachable routes.');
...
requireValue(!crawl.sitemapRoutes?.includes('/story/'), 'The prepared story route must not be misreported as present in the live sitemap snapshot.');
...
console.log(`Live search evidence valid: homepage and ${crawl.sitemapRouteCount}-route sitemap reachable, ${sample.queryCount} public searches sampled, no owned result observed, Search Console still unverified.`);
