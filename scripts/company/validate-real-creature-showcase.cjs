...
requireValue(release.state === 'withdrawn_visual_quality_failed_do_not_publish', 'renderer showcase must remain withdrawn from promotion');
...
for (const creature of manifest.creatures || []) {
    ...
    requireValue(!page.includes(creature.filename) && !page.includes(creature.id), `${creature.id} returned to the public page before visual approval`);
}

requireValue((page.match(/class="creature-specimen"/g) || []).length === 0, 'withdrawn specimen cards returned to the public page');
requireValue(page.includes('id="gene-spectrum"') && page.includes('Why the old sprite gallery is gone'), 'public page does not explain the creature system and withdrawal plainly');
...
requireValue(llms.includes('https://mythicalvoid.com/creature-genetics/#gene-spectrum'), 'machine-readable creature-engine discovery is missing');
...
requireValue(release.verification?.desktopVisualReviewPassed === false && release.verification?.phoneVisualReviewPassed === false, 'failed visual review must remain explicit');
