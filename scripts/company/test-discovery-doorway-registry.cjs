...
try {
    ...
    const failures = [
        ...
        ['missing-game-jolt-packet', value => { value.routes.find(route => route.id === 'game-jolt').preparedArtifact = null; }],
        ['game-jolt-ready', value => { value.routes.find(route => route.id === 'game-jolt').state = 'ready_waiting_for_kevin_action_time_approval'; }],
        ['game-jolt-no-thumbnail-gate', value => { value.routes.find(route => route.id === 'game-jolt').needs.approvedCatalogueThumbnail = false; }],
