...
requireValue(audit.scope?.freshFirstTimeGame?.completedThrough === 'hatching_started' && audit.scope?.freshFirstTimeGame?.meaningfulActionOrHatchReached === true && audit.scope?.freshFirstTimeGame?.phoneChecked === true, 'fresh first-time hatching evidence is incomplete');
requireValue(audit.scope?.freshFirstTimeGame?.savedPlayerDataInspected === false && audit.scope?.freshFirstTimeGame?.savedPlayerDataChanged === false && audit.scope?.freshFirstTimeGame?.existingPlayerDataChanged === false && audit.scope?.freshFirstTimeGame?.temporaryTestStateRetained === false, 'fresh first-time evidence crossed the saved-data boundary');
...
requireValue(audit.conversionRisks?.length === 4, 'four unresolved conversion risks are required');
for (const id of ['FI-001', 'FI-002', 'FI-003', 'FI-004']) requireValue(audit.conversionRisks?.some(item => item.id === id), `missing conversion risk ${id}`);
requireValue(!audit.conversionRisks?.some(item => item.id === 'FI-005'), 'resolved fresh-start risk must not remain open');
...
for (const phrase of ['This is now a game-presentation problem', 'Make the creature unmistakable', 'Fix the phone composition', 'Use “creature” consistently', 'Prove the returning beginning', '0 of 4 approved moments']) requireValue(copy.includes(phrase), `plain-language audit is missing: ${phrase}`);
