...
const page = read('public/space-discovery/index.html');
const client = read('public/space-discovery/space-discovery.js');
const styles = read('public/space-discovery/space-discovery.css');
const core = read('netlify/lib/space-discovery-core.cjs');
...
requireValue(page.includes("TODAY'S SPACE DISCOVERY"), 'page does not lead with the repeatable return reason');
...
requireValue(page.includes('data-share-url="https://mythicalvoid.com/space-discovery/"'), 'clean share URL is missing');
requireValue(page.includes('<script type="application/ld+json">'), 'learning resource structured data is missing');
requireValue(client.includes("fetch('/api/space-discovery'"), 'client does not use the owned cached endpoint');
requireValue(!/localStorage|sessionStorage/.test(client), 'Space Discovery should not store a visitor profile or return history');
...
requireValue(nasaPage.includes('href="/space-discovery/"'), 'NASA/STEM page does not open the daily discovery');
requireValue(storefront.includes("href=\"/space-discovery/\">Open today's Space Discovery"), 'homepage does not expose the return loop');
requireValue(sitemap.includes('<loc>https://mythicalvoid.com/space-discovery/</loc>') && sitemap.includes('<changefreq>daily</changefreq>'), 'sitemap does not describe the daily route');
requireValue(llms.includes('[Today\'s Space Discovery](https://mythicalvoid.com/space-discovery/)'), 'AI-readable guide omits the Space Discovery');
requireValue(netlify.includes('from = "/space-discovery/"') && netlify.includes('to = "/space-discovery/index.html"'), 'Netlify route is missing');
requireValue(netlify.includes('from = "/api/space-discovery"') && netlify.includes('to = "/.netlify/functions/space-discovery"'), 'plain-language API route is missing');
requireValue(vercel.includes('"source": "/space-discovery/"') && vercel.includes('"destination": "/space-discovery/index.html"'), 'Vercel route is missing');
