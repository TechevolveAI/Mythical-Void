...
requireValue(values.evidence.socialPreviewMetadata?.routeSpecificPageCount === 7 && values.evidence.socialPreviewMetadata?.storyUsesProjectBeaconWideArtwork === true && values.evidence.socialPreviewMetadata?.nasaPageUsesSpaceDiscoveryWideArtwork === true && values.evidence.socialPreviewMetadata?.studioUsesFatherSonStoryWideArtwork === true && values.evidence.socialPreviewMetadata?.imageDimensionsAndAltTextChecked === true && values.evidence.socialPreviewMetadata?.knownPressRoomStaticPreviewLimitationRecorded === true && values.evidence.socialPreviewMetadata?.trackingParametersPermitted === false && values.evidence.socialPreviewMetadata?.verified === true, 'Live evidence must retain the seven checked social previews, Project Beacon, NASA/STEM and founder-story artwork, and honest press-room limitation.');
...
requireValue(routeMap.get('/updates/')?.liveEntryCount === 8 && routeMap.get('/updates/')?.homepageLinked === true && routeMap.get('/updates/')?.sitemapListed === true, 'The live Signal Log must retain its eight checked notes, homepage link and sitemap entry.');
requireValue(routeMap.get('/updates/')?.rssFeedUrl === 'https://mythicalvoid.com/updates/feed.xml' && routeMap.get('/updates/')?.rssContentTypeObserved === 'application/rss+xml; charset=utf-8' && routeMap.get('/updates/')?.rssItemCount === 8, 'The live RSS feed must retain its owned URL, feed content type and eight checked items.');
requireValue(routeMap.get('/updates/')?.jsonFeedUrl === 'https://mythicalvoid.com/updates/feed.json' && routeMap.get('/updates/')?.jsonContentTypeObserved === 'application/feed+json; charset=utf-8' && routeMap.get('/updates/')?.jsonItemCount === 8 && routeMap.get('/updates/')?.feedFilesMatchedApprovedDigests === true, 'The live JSON feed must retain its owned URL, feed content type, eight checked items and exact-file proof.');
...
requireValue(values.signalLog.liveEntries?.length === 8 && values.signalLog.liveEntries?.every(entry => entry.destinationStatusObserved === 200), 'Signal Log must retain eight checked release destinations.');
...
requireValue(values.signalLog.qualityEvidence?.desktopVisualReviewPassed === true && values.signalLog.qualityEvidence?.phoneVisualReviewPassed === true && values.signalLog.qualityEvidence?.fullTestSuitesPassed === 170 && values.signalLog.qualityEvidence?.fullTestsPassed === 1445, 'Signal Log must retain its visual and full-test evidence.');
requireValue(values.signalLog.draftAutomation?.state === 'merged_to_main_internal_review_only' && values.signalLog.draftAutomation?.sourceEntryCount === 8 && values.signalLog.draftAutomation?.reviewDraftCount === 24 && values.signalLog.draftAutomation?.safeguardFailureCasesPassed === 12 && values.signalLog.draftAutomation?.externalActionPerformed === false, 'Signal Log draft automation must retain its merged, tested and internal-only state.');
...
requireValue(values.signalLog.signalLogSyndicationRelease?.state === 'published_to_owned_website_and_live_verified' && values.signalLog.signalLogSyndicationRelease?.productionPullRequest === 'https://github.com/TechevolveAI/Mythical-Void/pull/92' && values.signalLog.signalLogSyndicationRelease?.itemCount === 8 && values.signalLog.signalLogSyndicationRelease?.sourceBoundBuild === true, 'Signal feed release must retain its live production record, eight-item count and source-bound build.');
