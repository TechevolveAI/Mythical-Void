...
requireValue(routeMap.get('/press/')?.socialAssetCount === 6 && routeMap.get('/press/')?.socialArtworkLinked === true, 'The live press room must retain all six linked social images.');
requireValue(routeMap.get('/updates/')?.liveEntryCount === 9 && routeMap.get('/updates/')?.homepageLinked === true && routeMap.get('/updates/')?.sitemapListed === true, 'The live Signal Log must retain its nine checked notes, homepage link and sitemap entry.');
requireValue(routeMap.get('/updates/')?.rssFeedUrl === 'https://mythicalvoid.com/updates/feed.xml' && routeMap.get('/updates/')?.rssContentTypeObserved === 'application/rss+xml; charset=utf-8' && routeMap.get('/updates/')?.rssItemCount === 9, 'The live RSS feed must retain its owned URL, feed content type and nine checked items.');
requireValue(routeMap.get('/updates/')?.jsonFeedUrl === 'https://mythicalvoid.com/updates/feed.json' && routeMap.get('/updates/')?.jsonContentTypeObserved === 'application/feed+json; charset=utf-8' && routeMap.get('/updates/')?.jsonItemCount === 9 && routeMap.get('/updates/')?.feedFilesMatchedApprovedDigests === true, 'The live JSON feed must retain its owned URL, feed content type, nine checked items and exact-file proof.');
...
requireValue(values.signalLog.liveEntries?.length === 9 && values.signalLog.liveEntries?.every(entry => entry.destinationStatusObserved === 200), 'Signal Log must retain nine checked release destinations.');
...
requireValue(values.signalLog.qualityEvidence?.desktopVisualReviewPassed === true && values.signalLog.qualityEvidence?.phoneVisualReviewPassed === true && values.signalLog.qualityEvidence?.playableNowSafeguardCasesPassed === 15 && values.signalLog.qualityEvidence?.fullTestSuitesPassed === 180 && values.signalLog.qualityEvidence?.fullTestsPassed === 1653, 'Signal Log must retain its visual, Playable Now safeguard and full-test evidence.');
requireValue(values.signalLog.draftAutomation?.state === 'merged_to_main_internal_review_only' && values.signalLog.draftAutomation?.sourceEntryCount === 9 && values.signalLog.draftAutomation?.reviewDraftCount === 27 && values.signalLog.draftAutomation?.safeguardFailureCasesPassed === 12 && values.signalLog.draftAutomation?.externalActionPerformed === false, 'Signal Log draft automation must retain its merged, tested and internal-only state.');
...
requireValue(values.signalLog.signalLogSyndicationRelease?.state === 'published_to_owned_website_and_live_verified' && values.signalLog.signalLogSyndicationRelease?.productionPullRequest === 'https://github.com/TechevolveAI/Mythical-Void/pull/102' && values.signalLog.signalLogSyndicationRelease?.itemCount === 9 && values.signalLog.signalLogSyndicationRelease?.sourceBoundBuild === true, 'Signal feed release must retain its live production record, nine-item count and source-bound build.');
