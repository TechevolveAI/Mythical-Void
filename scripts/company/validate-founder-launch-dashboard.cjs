...
requireValue(values.signalLog.draftAutomation?.state === 'merged_to_main_internal_review_only' && values.signalLog.draftAutomation?.sourceEntryCount === 2 && values.signalLog.draftAutomation?.reviewDraftCount === 6 && values.signalLog.draftAutomation?.safeguardFailureCasesPassed === 11 && values.signalLog.draftAutomation?.externalActionPerformed === false, 'Signal Log draft automation must retain its merged, tested and internal-only state.');
