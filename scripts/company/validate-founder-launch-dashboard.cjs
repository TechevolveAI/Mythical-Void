...
requireValue(routeMap.get('/updates/')?.liveEntryCount === 4 && routeMap.get('/updates/')?.homepageLinked === true && routeMap.get('/updates/')?.sitemapListed === true, 'The live Signal Log must retain its four checked notes, homepage link and sitemap entry.');
...
requireValue(values.signalLog.liveEntries?.length === 4 && values.signalLog.liveEntries?.every(entry => entry.destinationStatusObserved === 200), 'Signal Log must retain four checked release destinations.');
...
requireValue(values.signalLog.draftAutomation?.state === 'merged_to_main_internal_review_only' && values.signalLog.draftAutomation?.sourceEntryCount === 4 && values.signalLog.draftAutomation?.reviewDraftCount === 12 && values.signalLog.draftAutomation?.safeguardFailureCasesPassed === 12 && values.signalLog.draftAutomation?.externalActionPerformed === false, 'Signal Log draft automation must retain its merged, tested and internal-only state.');
