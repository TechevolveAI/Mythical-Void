...
requireValue(routeMap.get('/press/')?.socialAssetCount === 4 && routeMap.get('/press/')?.socialArtworkLinked === true, 'The live press room must retain all four linked social images.');
requireValue(routeMap.get('/updates/')?.liveEntryCount === 6 && routeMap.get('/updates/')?.homepageLinked === true && routeMap.get('/updates/')?.sitemapListed === true, 'The live Signal Log must retain its six checked notes, homepage link and sitemap entry.');
...
requireValue(values.signalLog.liveEntries?.length === 6 && values.signalLog.liveEntries?.every(entry => entry.destinationStatusObserved === 200), 'Signal Log must retain six checked release destinations.');
...
requireValue(values.signalLog.draftAutomation?.state === 'merged_to_main_internal_review_only' && values.signalLog.draftAutomation?.sourceEntryCount === 6 && values.signalLog.draftAutomation?.reviewDraftCount === 18 && values.signalLog.draftAutomation?.safeguardFailureCasesPassed === 12 && values.signalLog.draftAutomation?.externalActionPerformed === false, 'Signal Log draft automation must retain its merged, tested and internal-only state.');
