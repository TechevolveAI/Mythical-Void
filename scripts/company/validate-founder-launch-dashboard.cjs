...
requireValue(routeMap.get('/updates/')?.liveEntryCount === 7 && routeMap.get('/updates/')?.homepageLinked === true && routeMap.get('/updates/')?.sitemapListed === true, 'The live Signal Log must retain its seven checked notes, homepage link and sitemap entry.');
...
requireValue(values.signalLog.liveEntries?.length === 7 && values.signalLog.liveEntries?.every(entry => entry.destinationStatusObserved === 200), 'Signal Log must retain seven checked release destinations.');
...
requireValue(values.signalLog.draftAutomation?.state === 'merged_to_main_internal_review_only' && values.signalLog.draftAutomation?.sourceEntryCount === 7 && values.signalLog.draftAutomation?.reviewDraftCount === 21 && values.signalLog.draftAutomation?.safeguardFailureCasesPassed === 12 && values.signalLog.draftAutomation?.externalActionPerformed === false, 'Signal Log draft automation must retain its merged, tested and internal-only state.');
