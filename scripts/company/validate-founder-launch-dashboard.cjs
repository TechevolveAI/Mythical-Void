...
requireValue(routeMap.get('/press/')?.socialAssetCount === 6 && routeMap.get('/press/')?.socialArtworkLinked === true && routeMap.get('/press/')?.authenticSocialVideoCount === 3 && routeMap.get('/press/')?.socialVideoCaptionDraftCount === 3 && routeMap.get('/press/')?.socialVideoKitLinked === true && routeMap.get('/press/')?.socialVideoFilesMatchedApprovedDigests === true && routeMap.get('/press/')?.externalSocialPublicationAuthorized === false, 'The live press room must retain all six social images, three authentic-gameplay videos, three caption drafts, exact-file proof and its external-publication gate.');
requireValue(routeMap.get('/updates/')?.liveEntryCount === 10 && routeMap.get('/updates/')?.homepageLinked === true && routeMap.get('/updates/')?.sitemapListed === true, 'The live Signal Log must retain its ten checked notes, homepage link and sitemap entry.');
requireValue(routeMap.get('/updates/')?.rssFeedUrl === 'https://mythicalvoid.com/updates/feed.xml' && routeMap.get('/updates/')?.rssContentTypeObserved === 'application/rss+xml; charset=utf-8' && routeMap.get('/updates/')?.rssItemCount === 10, 'The live RSS feed must retain its owned URL, feed content type and ten checked items.');
requireValue(routeMap.get('/updates/')?.jsonFeedUrl === 'https://mythicalvoid.com/updates/feed.json' && routeMap.get('/updates/')?.jsonContentTypeObserved === 'application/feed+json; charset=utf-8' && routeMap.get('/updates/')?.jsonItemCount === 10 && routeMap.get('/updates/')?.feedFilesMatchedApprovedDigests === true, 'The live JSON feed must retain its owned URL, feed content type, ten checked items and exact-file proof.');
...
requireValue(values.signalLog.liveEntries?.length === 10 && values.signalLog.liveEntries?.every(entry => entry.destinationStatusObserved === 200), 'Signal Log must retain ten checked release destinations.');
...
requireValue(values.signalLog.qualityEvidence?.desktopVisualReviewPassed === true && values.signalLog.qualityEvidence?.phoneVisualReviewPassed === true && values.signalLog.qualityEvidence?.playableNowSafeguardCasesPassed === 15 && values.signalLog.qualityEvidence?.authenticGameplaySocialKitSafeguardCasesPassed === 17 && values.signalLog.qualityEvidence?.fullTestSuitesPassed === 181 && values.signalLog.qualityEvidence?.fullTestsPassed === 1655, 'Signal Log must retain its visual, Playable Now, social-video safeguard and full-test evidence.');
requireValue(values.signalLog.draftAutomation?.state === 'merged_to_main_internal_review_only' && values.signalLog.draftAutomation?.sourceEntryCount === 10 && values.signalLog.draftAutomation?.reviewDraftCount === 30 && values.signalLog.draftAutomation?.safeguardFailureCasesPassed === 12 && values.signalLog.draftAutomation?.externalActionPerformed === false, 'Signal Log draft automation must retain its merged, tested and internal-only state.');
...
requireValue(values.signalLog.signalLogSyndicationRelease?.state === 'published_to_owned_website_and_live_verified' && values.signalLog.signalLogSyndicationRelease?.productionPullRequest === 'https://github.com/TechevolveAI/Mythical-Void/pull/103' && values.signalLog.signalLogSyndicationRelease?.itemCount === 10 && values.signalLog.signalLogSyndicationRelease?.sourceBoundBuild === true, 'Signal feed release must retain its live production record, ten-item count and source-bound build.');
requireValue(values.signalLog.signalLogSyndicationRelease?.rssFeedUrl === 'https://mythicalvoid.com/updates/feed.xml' && values.signalLog.signalLogSyndicationRelease?.rssContentTypeObserved === 'application/rss+xml; charset=utf-8' && /^[0-9a-f]{64}$/.test(values.signalLog.signalLogSyndicationRelease?.rssSha256 || ''), 'RSS release must retain its owned URL, feed content type and exact-file proof.');
requireValue(values.signalLog.signalLogSyndicationRelease?.jsonFeedUrl === 'https://mythicalvoid.com/updates/feed.json' && values.signalLog.signalLogSyndicationRelease?.jsonContentTypeObserved === 'application/feed+json; charset=utf-8' && /^[0-9a-f]{64}$/.test(values.signalLog.signalLogSyndicationRelease?.jsonSha256 || ''), 'JSON release must retain its owned URL, feed content type and exact-file proof.');
