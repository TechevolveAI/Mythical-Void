...
requireValue(values.evidence.socialPreviewMetadata?.routeSpecificPageCount === 7 && values.evidence.socialPreviewMetadata?.storyUsesProjectBeaconWideArtwork === true && values.evidence.socialPreviewMetadata?.imageDimensionsAndAltTextChecked === true && values.evidence.socialPreviewMetadata?.knownPressRoomStaticPreviewLimitationRecorded === true && values.evidence.socialPreviewMetadata?.trackingParametersPermitted === false && values.evidence.socialPreviewMetadata?.verified === true, 'Live evidence must retain the seven checked social previews, Project Beacon story artwork and honest press-room limitation.');
...
requireValue(routeMap.get('/press/')?.socialAssetCount === 2 && routeMap.get('/press/')?.socialArtworkLinked === true, 'The live press room must retain both linked Project Beacon social images.');
requireValue(routeMap.get('/updates/')?.liveEntryCount === 5 && routeMap.get('/updates/')?.homepageLinked === true && routeMap.get('/updates/')?.sitemapListed === true, 'The live Signal Log must retain its five checked notes, homepage link and sitemap entry.');
...
requireValue(values.signalLog.liveEntries?.length === 5 && values.signalLog.liveEntries?.every(entry => entry.destinationStatusObserved === 200), 'Signal Log must retain five checked release destinations.');
...
requireValue(values.signalLog.draftAutomation?.state === 'merged_to_main_internal_review_only' && values.signalLog.draftAutomation?.sourceEntryCount === 5 && values.signalLog.draftAutomation?.reviewDraftCount === 15 && values.signalLog.draftAutomation?.safeguardFailureCasesPassed === 12 && values.signalLog.draftAutomation?.externalActionPerformed === false, 'Signal Log draft automation must retain its merged, tested and internal-only state.');
