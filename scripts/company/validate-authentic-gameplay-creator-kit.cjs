...
function validate(values) {
    ...
    requireValue(signal.entries?.[0]?.id === 'SIGNAL-011' && signalEntry?.destination === '/press/#creator-download-kit' && signalEntry?.status === 'live' && /No external post or message has been sent/i.test(signalEntry?.details?.[2] || ''), 'Signal Log must lead with the creator handoff and retain the no-external-action boundary');
