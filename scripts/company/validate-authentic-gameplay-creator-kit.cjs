...
function validate(values) {
    ...
    requireValue(!pressSource.includes('id="creator-download-kit"') && pressSource.includes('The public media library is being rebuilt.') && pressSource.includes('No gameplay download pack is approved.'), 'press room still exposes the withdrawn creator package');
    requireValue(!llms.includes('One-download creator kit:'), 'machine-readable guide still exposes the withdrawn creator package');
    const signalEntry = signal.entries?.find(entry => entry.id === 'UPDATE-011');
    requireValue(!signalEntry, 'Latest News still promotes the withdrawn creator package');
