...
requireValue(kit.asOf === '2026-09-08', 'kit review date is stale');
...
requireValue(linkedIn?.publicIdentity?.publicUrlCandidate === 'linkedin.com/company/play-mythical-void', 'LinkedIn primary address drifted');
requireValue(linkedIn?.publicIdentity?.fallbackPublicUrlCandidate === 'linkedin.com/company/mythical-void-game', 'LinkedIn fallback address drifted');
...
requireValue(youtube?.publicIdentity?.handleCandidate === '@PlayMythicalVoid', 'YouTube primary handle drifted');
requireValue(youtube?.publicIdentity?.fallbackHandleCandidate === '@MythicalVoidGame', 'YouTube fallback handle drifted');
