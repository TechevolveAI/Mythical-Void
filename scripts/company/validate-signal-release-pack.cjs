...
for (const item of pack.items || []) {
    ...
    if (entry?.imageClass === 'branded_renderer_proof_layout_with_authentic_game_sprite_exports') requireValue(/branded proof layout/i.test(item.media?.disclosure || '') && /not a playable game scene/i.test(item.media?.disclosure || '') && /exact game-renderer export/i.test(item.media?.disclosure || '') && /does not represent normal hatch odds/i.test(item.media?.disclosure || ''), `${label} creature renderer proof needs its layout, exact-export and selection boundaries.`);
