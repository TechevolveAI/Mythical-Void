...
const clean = {
    ...
    playHtml: `<title>Play Mythical Void Free | Alien Creature Browser Game</title><script>var path='/play'; var isGameRoute = path === '/play'; if (isGameRoute) return; window.MYTHICAL_GOOGLE_TAG_ID='G-FTM4W73ECQ';</script>`,
...
cases += 1; assert.match(inspectCorePages({ ...clean, playHtml: '<title>Play Mythical Void Free | Alien Creature Browser Game</title><script src="https://www.googletagmanager.com/gtag/js?id=G-FTM4W73ECQ"></script>' }).errors.join('\n'), /leaked into the game/);
