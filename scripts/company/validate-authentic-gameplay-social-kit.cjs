...
function validate(values, baseDir = root) {
    ...
    requireValue(manifest.state === 'owned_press_room_release_waiting_for_external_channel_and_kevin_approval', 'social kit must remain limited to the owned press room while waiting for a channel and Kevin');
    requireValue(manifest.source?.sha256 === expected.sourceSha256 && sha256(sourcePath) === expected.sourceSha256, 'authentic source video fingerprint changed');
    ...
    requireValue(sourceManifest.asset?.sha256 === expected.sourceSha256 && sourceManifest.ownedWebsiteProofUseAuthorized === true && sourceManifest.externalPromotionAuthorized === false && sourceManifest.kevinApprovalRequiredBeforeExternalPublication === true, 'source authority must permit owned proof while retaining the external promotion gate');
    ...
    requireValue(captions.state === 'source_bound_drafts_waiting_for_channel_and_kevin_approval' && captions.cleanDestination === 'https://mythicalvoid.com/playable-now/', 'caption pack must remain source-bound, clean-linked and waiting for approval');
    ...
    requireValue(captions.authority?.publicPressRoomPublicationAuthorized === true && captions.authority?.externalSocialPublicationAuthorized === false && captions.authority?.creatorOutreachSendingAuthorized === false && captions.authority?.paidPromotionAuthorized === false && captions.authority?.publicRepliesAuthorized === false && captions.authority?.kevinApprovalRequiredBeforeExternalPublication === true && captions.authority?.externalActionPerformed === false, 'caption authority must keep external publication, sending, spending and replies closed');

    requireValue(manifest.authority?.ownedPressRoomPublicationAuthorized === true && manifest.authority?.externalSocialPublicationAuthorized === false && manifest.authority?.creatorOutreachSendingAuthorized === false && manifest.authority?.paidPromotionAuthorized === false && manifest.authority?.publicRepliesAuthorized === false && manifest.authority?.kevinApprovalRequiredBeforeExternalPublication === true && manifest.authority?.externalActionPerformed === false, 'media authority must keep external publication, sending, spending and replies closed');
    requireValue(release.state === 'owned_press_room_release_visually_verified_waiting_for_production_verification' && release.verification?.allMediaFingerprintsMatched === true && release.verification?.deterministicRebuildManifestMatched === true && release.verification?.posterVisualReviewsPassed === 3 && release.verification?.motionContactSheetReviewPassed === true && release.verification?.squareTextClippingObserved === false && release.verification?.desktopPressRoomVisualReviewPassed === true && release.verification?.phonePressRoomVisualReviewPassed === true && release.verification?.phoneWidth === 390 && release.verification?.phoneHorizontalOverflowObserved === false && release.verification?.browserVideoCount === 3 && release.verification?.browserVideoDurationSeconds === 19.583333 && release.verification?.captionActionsVisibleOnPhone === true && release.verification?.fullTestSuitesPassed === 181 && release.verification?.fullTestsPassed === 1655 && release.verification?.productionUrlsVerified === false, 'release must retain completed media, reproducible build, press-room and full-test review without inventing production proof');
    ...
    requireValue(release.authority?.ownedPressRoomPublicationAuthorized === true && release.authority?.externalSocialPublicationAuthorized === false && release.authority?.creatorOutreachSendingAuthorized === false && release.authority?.paidPromotionAuthorized === false && release.authority?.publicRepliesAuthorized === false && release.authority?.kevinApprovalRequiredBeforeExternalPublication === true && release.authority?.externalActionPerformed === false, 'release authority silently widened');
    ...
    requireValue(pressAssets.authenticGameplaySocialKit?.manifest === 'https://mythicalvoid.com/press/social-video/manifest.json' && pressAssets.authenticGameplaySocialKit?.captionPack === 'https://mythicalvoid.com/press/social-video/authentic-gameplay-caption-pack.json' && pressAssetsVideos.length === 3, 'press manifest must list the kit, caption pack and three social videos');
