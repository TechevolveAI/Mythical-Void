...
requireValue(hatchChallenge.visualBoundary?.newArtworkUsed === true && ['source_ready_for_owned_release', 'live_on_production_alias_from_promoted_deploy_preview'].includes(hatchChallenge.visualBoundary?.newArtworkState) && hatchChallenge.visualBoundary?.newArtworkKind === 'ai_assisted_code_authored_brand_art_not_gameplay' && hatchChallenge.visualBoundary?.imageModelUsed === false && hatchChallenge.visualBoundary?.gameplayMediaUsed === false && hatchChallenge.visualBoundary?.visualLaunchGateChanged === false, 'Hatch Challenge visual boundary drifted');
if (hatchChallenge.visualBoundary?.newArtworkState === 'live_on_production_alias_from_promoted_deploy_preview') {
    const proof = hatchChallenge.visualBoundary.liveAliasProof || {};
    requireValue(/^[0-9a-f]{40}$/.test(proof.sourceCommit || '') && /^[0-9a-f]{40}$/.test(proof.mainMergeCommit || '') && /^[0-9a-f]{24}$/.test(proof.deployId || ''), 'Hatch Challenge live-alias proof is missing');
    requireValue(proof.deployContext === 'deploy-preview' && proof.promotionMethod === 'restoreSiteDeploy' && proof.productionAlias === 'https://mythicalvoid.com', 'Hatch Challenge promoted preview context drifted');
    requireValue(proof.liveImageSha256 === '5030a304f7f81b5b7d6284a0100efaef35eec1ff534db0744adbf4de5a7662f7' && proof.liveBrowserErrorCount === 0, 'Hatch Challenge live-alias verification drifted');
}
